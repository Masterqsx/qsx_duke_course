...
3. Program explanation
  1)  "main_define.cpp" is the main entrance of the whole program, it handles command line argument and build global variables. You can choose file input or standard input to enter command
  2)  "parsing.h" desides which command the input belongs to.
  3)  "define_parsing.h" handle "define" command
  4)  "eval_parsing.h"   handle "evaluate" command
  5)  "numint_parsing.h" handle "numint" command
  6)  "mcint_parsing.h"  handle "mcint" command
  7)  "gradient_parsing.h" handle "max" and "min" command
  8)  "equa_cal.h" handle the evaluation of function which supports the realization of "numint" "mcint" "max" and "min"
  9)  "gradient_cal.h"   handle the deriviative which supports "max" and "min"
  10) "check_list.h"     handle most of input type check and new Nodes creation
  11) "equation_tree.h"  is the most important data structure of whole program. It is a equation tree, which could store constant Nodes, variable Nodes, symbol Nodes('+','-'....) and equation Nodes.
 
4. Test case explanation (When meet invalid input would abort, avoid seg fault) 
